#!/usr/bin/env python3
"""
derive_book.py  (v2 — VERIFIED boundaries)

Recover the empty `book` column from the citation page/section/line, using
exact book-opening loci looked up against Burnet (Stephanus) and Bekker.
...
Usage:
    python3 derive_book.py resolved.tsv > resolved_with_books.tsv
    python3 derive_book.py resolved.tsv --audit
"""
import sys, csv, re
...
def build_intervals(work):
    ...
    # of >1 between consecutive openings' pages. This stops a book at the end of
    ...
    # Explicit end-of-run caps: the last book in a run ends at the run's own
    # textual close, NOT at the next run's opening (the inter-run gap belongs to
    # neither). For EE the common-book run (IV*/V*/VI*, borrowed from NE
    # pagination) closes where NE's common books close: NE VIII opens 1155a3.
...
def assign(work, match_cell):
    ...
    # Bare page+section (no line): a book opening mid-page at, say, 1094a1 or
    # 980a21 still OWNS all of column 'a' from the reader's standpoint — a cite
    # '1094a' means "NE book I", not "the sliver of a before line 1". So when
    # there is no line, probe at the TOP of the section (line 0) AND, if that
    # lands below the first book that touches this page/section, snap up to it.
    if line is None:
        # Probe the top of the cited section. Whichever book's half-open span
        # contains (page, sec, 0) owns a bare page+section cite. If that key
        # falls below all intervals (because the first book on this page opens
        # mid-page/mid-section), snap to the earliest book that opens on this
        # page. This makes '1094a'->I, '980a'->Alpha, '1214a'->EE I, while a
        # bare cite to a page where a NEW book opens mid-section (e.g. '1103a',
        # II opens 1103a15) resolves to the book owning the TOP of that section
        # (I), deterministically. Such cases are vanishingly rare and, being
        # genuinely line-ambiguous, are documented as boundary-approximate.
        k = (page, sec or "a", 0)
        ...
        if cands:
            label = min(cands)[1]
            return (label, label.endswith("*"))
        # no book opens on this page and k is below all spans: assign the book
        # whose span will contain this page (the first interval starting after
        # is wrong; take the last interval whose lo <= (page, 'z', inf))
...
def main():
    ...
    with open(path, encoding="utf-8", newline="") as fh:
        ...
        for row in r:
            if len(row) > max(WORK,MATCH,BOOK) and row[WORK] in faceted:
                total += 1
